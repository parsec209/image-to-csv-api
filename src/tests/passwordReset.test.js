...
const postUser = async () => {
  user = await request(app)
    ...
    .send({ username: uuidv4(), email: `${uuidv4()}@test.com`, password: '11111Aab', confirmedPassword: '11111Aab' })
  return user.body
}
...
describe('sending reset email', () => {
  test('successfully sends reset email', async () => {
    await expect(sendResetEmail(user.email, 'testToken', 'https://testURL.com/reset/')).resolves.not.toThrow()
  })
})
