...
const generateCreds = () => {
  const creds = { username: uuidv4(), email: `${uuidv4()}@test.com`, password: '11111Aab', confirmedPassword: '11111Aab' }
  return creds
}
...
describe('POST /api/user/register', () => {
  ...
  test('handles 11000 error code (i.e. email already exists)', async () => {
    ...
    const error = await request(app)
      .post('/api/user/register')
      .send({ username: 'newUser123434980', email: creds.email, password: creds.password, confirmedPassword: creds.confirmedPassword })
...
describe('POST /api/user/login', () => {
  const { username, email, password, confirmedPassword } = generateCreds()

  beforeAll(async () => {
    await request(app)
      .post('/api/user/register')
      .send({ username, email, password, confirmedPassword })
  })
...
describe('POST /api/user/forgot', () => {
  ...
  test('responds with success status code after email sent', async () => {
    const response = await request(app)
      ...
      .send({ email, resetURL: 'https://testURL.com/reset/' })
