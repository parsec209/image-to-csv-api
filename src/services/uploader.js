...
const checkFileTypeFromBuffer = require('file-type').fromBuffer    //file-type package versions after 14.7.1 only use ESM
...
class Uploader {
  ...
  async checkFileFormats() {
    await Promise.all(this.files.map(async function(file) {
      const fileType = await checkFileTypeFromBuffer(file.buffer)
