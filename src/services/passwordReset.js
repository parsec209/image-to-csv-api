 ...
 * @returns {Promise<void>}
 */
const sendResetEmail = async function(recipient, token, resetURL) {
  const argsValidationSchema = Joi.object({ 
    0: Joi.string().required(), 
    1: Joi.string().required(), 
    2: Joi.string() 
  })
  const validation = argsValidationSchema.validate(arguments)
  if (validation.hasOwnProperty('error')) {
    throw new Error(JSON.stringify(result.error))
  }
  const URL = resetURL ? resetURL : ''
  const body = 'You are receiving this email because you have requested a password reset for your Image-To-CSV account.\n\n' +
    'Please either click on the following link, or use the following token, to complete the password reset process:\n\n' +
    `${URL}${token}\n\n` +
